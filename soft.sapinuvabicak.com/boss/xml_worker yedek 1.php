...
if (!function_exists('imagewebp')) {
    // WebP desteği yoksa hata logla
    file_put_contents('xml_worker_error.txt', "UYARI: WebP desteği bulunamadı. Resimler orijinal formatlarında kaydedilecek.\n", FILE_APPEND);
    $webp_support = false;
} else {
    $webp_support = true;
    file_put_contents('xml_worker_error.txt', "WebP desteği aktif. Resimler WebP formatında kaydedilecek.\n", FILE_APPEND);
}
...
if (!$is_cron_job) {
    ...
    $allowed_ips = ['127.0.0.1', '::1', 'localhost']; // Sadece localhost'tan cron job çalışabilir
    ...
    $expected_token = 'xml_cron_2024_secure'; // Bu token'ı değiştirin
...
function updateExistingProduct($ozy, $product_data, $product_id) {
    try {
        $stmt = $ozy->prepare("UPDATE urunler SET adi=?, seo=?, urunkodu=?, urunbarkodu=?, stok=?, fiyat=?, kdv=?, aciklama=?, kategori=?, marka=?, resim=? WHERE id=?");
        $result = $stmt->execute([
            $product_data['adi'],
            $product_data['seo'], 
...
function processBatchProducts($ozy, $batch_products, $batch_images)
{
    ...
    try {
        ...
        // Ürünleri bulk insert
        $stmt = $ozy->prepare("INSERT INTO urunler (adi, seo, urunkodu, urunbarkodu, parabirimi, stok, fiyat, kdv, aciklama, kategori, durum, tarih, yildiz, resim, marka) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        foreach ($batch_products as $product) {
            ...
            if ($result) {
                ...
                file_put_contents('xml_bulk_error.txt', "Bulk insert hatası: " . $error[2] . "\n", FILE_APPEND);
...
// WebP dönüştürme fonksiyonu - Agresif optimizasyon
function convertToWebP($imageData, $quality = 50) {
    // WebP desteği kontrolü
    if (!function_exists('imagewebp')) {
        return false;
    }
    ...
    // Çok küçük resimler için WebP dönüştürme yapma (agresif optimizasyon)
    if ($width < 300 || $height < 300) {
        return $imageData;
    }
    
    // Çok büyük resimler için boyut sınırla (memory tasarrufu)
    $maxSize = 2000;
    ...
    // Zaten WebP formatında ise dönüştürme
    ...
    // Boyut değişikliği gerekliyse resize yap
    if ($newWidth != $width || $newHeight != $height) {
        ...
        if ($mimeType === 'image/png') {
            ...
            imagefill($resizedResource, 0, 0, $transparent);
        }
        
        imagecopyresampled($resizedResource, $imageResource, 0, 0, 0, 0, $newWidth, $newHeight, $width, $height);
        imagedestroy($imageResource);
        $imageResource = $resizedResource;
    }
    
    // WebP'ye dönüştür - daha düşük kalite ile
    ob_start();
    $success = imagewebp($imageResource, null, $quality);
...
// cURL ile resim indirme fonksiyonu - WebP dönüştürme ile
function downloadImageWithCurl($url, $convertToWebP = true)
{
    // URL cache kontrolü - aynı resim tekrar indirilmesin (512MB limit için)
    static $imageCache = [];
    static $cacheCount = 0;
    $cacheKey = md5($url . ($convertToWebP ? '_webp' : '_orig'));
    ...
    curl_setopt($ch, CURLOPT_TIMEOUT, 2); // 2 saniye timeout (3'ten düşürüldü - agresif optimizasyon)
    curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 1); // 1 saniye bağlantı timeout (2'den düşürüldü - agresif optimizasyon)
    ...
    // WebP'ye dönüştür - sadece çok büyük resimler için (agresif optimizasyon)
    if ($convertToWebP && $imageData) {
        // Resim boyutunu kontrol et - sadece 500x500'den büyük resimler için WebP
        $imageInfo = @getimagesizefromstring($imageData);
...
// Eş zamanlı resim indirme fonksiyonu - WebP dönüştürme ile
function downloadImagesConcurrently($urls, $maxConcurrent = 10, $convertToWebP = true)
{
    if (empty($urls)) {
        return [];
    }
    ...
    foreach ($urlBatches as $batch) {
        ...
        foreach ($batch as $index => $url) {
            ...
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
            curl_setopt($ch, CURLOPT_TIMEOUT, 2); // 2 saniye timeout (5'ten düşürüldü - agresif optimizasyon)
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 1); // 1 saniye bağlantı timeout (3'ten düşürüldü - agresif optimizasyon)
            curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36');
        ...
        // Sonuçları al ve WebP'ye dönüştür
        foreach ($curlHandles as $index => $ch) {
            ...
            if ($error || $httpCode !== 200 || $fileSize > 3145728) {
                ...
                // Cache'e ekle
                $cacheKey = md5($batch[$index] . ($convertToWebP ? '_webp' : '_orig'));
                $imageCache[$cacheKey] = $imageData;
                $cacheCount++;
            }
...
// Trafik kaydetme fonksiyonu
function saveTrafficData($ozy, $islem, $kullanim, $tarih = null) {
    try {
        if ($tarih === null) {
            $tarih = date('Y-m-d');
        }
        ...
        if ($result) {
            file_put_contents('xml_worker_error.txt', "Trafik verisi kaydedildi: {$islem} - {$kullanim} MB - {$tarih}\n", FILE_APPEND);
            return true;
        } else {
            file_put_contents('xml_worker_error.txt', "Trafik verisi kaydedilemedi: " . json_encode($stmt->errorInfo()) . "\n", FILE_APPEND);
            return false;
        }
    } catch (Exception $e) {
        file_put_contents('xml_worker_error.txt', "Trafik kaydetme hatası: " . $e->getMessage() . "\n", FILE_APPEND);
        return false;
    }
}
...
if ($xml_id > 0) {
    ...
    try {
        ...
        $batch_size = 20; // 20 ürünlük batch'ler halinde işle (agresif optimizasyon)
        $batch_products = [];
        $batch_images = [];
        ...
        // Detaylı hız logları için değişkenler
        $product_times = [];
        $image_times = [];
        $webp_times = [];
        $db_times = [];
        ...
        file_put_contents('xml_progress.txt', "Batch Size: {$batch_size} (Agresif Optimizasyon)\n", FILE_APPEND);
        file_put_contents('xml_progress.txt', "Eş Zamanlı İndirme: 3 (Agresif Optimizasyon)\n", FILE_APPEND);
        file_put_contents('xml_progress.txt', "WebP Dönüştürme: Sadece 500x500+ resimler\n", FILE_APPEND);
        ...
        foreach ($xml->children() as $urun) {
            try {
                // Ürün işleme başlangıcı - detaylı hız ölçümü
                $product_start_time = microtime(true);
                
                if ($i % 10 == 0) { // Her 10 üründe bir log (daha sık)
                    $memory_usage = memory_get_usage(true) / 1024 / 1024; // MB
                    $elapsed = microtime(true) - $start_time;
                    $avg_time_per_product = $elapsed / $i;
                    $estimated_remaining = ($total_products - $i) * $avg_time_per_product;
                    $estimated_remaining_formatted = gmdate('H:i:s', $estimated_remaining);
                    
                    file_put_contents('xml_worker_error.txt', "Ürün {$i}/{$total_products} işleniyor... Memory: {$memory_usage}MB | Ortalama: " . round($avg_time_per_product, 3) . "s/ürün | Kalan: {$estimated_remaining_formatted}\n", FILE_APPEND);
                }
                // Progress güncelleme
                if ($i % 10 == 0 || $i == 1) {
                }
                ...
                if ($resimtip == '0') { // Tüm ürünler için resim indir
                    ...
                    if (!empty($resimkonum) && $resimkonum != " " && filter_var($resimkonum, FILTER_VALIDATE_URL)) {
                        ...
                        // WebP desteği varsa .webp, yoksa orijinal uzantı
                        $yeniad = rand() . md5(time() . $resimad) . ($webp_support ? '.webp' : ($uzanti ? '.' . $uzanti : '.jpg'));
                ...
                if ($resimtip == '1') { // Tüm ürünler için ek resim işle
                    ...
                    if (!empty($anaresim)) {
                        ...
                        for ($idx = 1; $idx <= 9; $idx++) {
                            if (isset($sonuc[$idx]) && $sonuc[$idx] != " ") {
                                ...
                                $uzanti = pathinfo($resimad, PATHINFO_EXTENSION);
                                // WebP desteği varsa .webp, yoksa orijinal uzantı
                                $ek_yeniad = rand() . md5(time() . $resimad . $idx) . ($webp_support ? '.webp' : ($uzanti ? '.' . $uzanti : '.jpg'));
                    ...
                    foreach ($urun as $field_name => $field_value) {
                        ...
                        if (
                            ...
                            // WebP desteği varsa .webp, yoksa orijinal uzantı
                            $ek_yeniad = 'img' . $unique_id . '_' . rand() . md5(time() . $resimad) . ($webp_support ? '.webp' : ($uzanti ? '.' . $uzanti : '.jpg'));
                            $yol = "../resimler/urunler";
                ...
                if (!empty($imageUrls)) {
                    try {
                        ...
                        $downloadedImages = downloadImagesConcurrently($imageUrls, 3, $webp_support); // WebP desteği ile eş zamanlı indirme (8'den 3'e düşürüldü - agresif optimizasyon)
                        $image_end_time = microtime(true);
                        $image_duration = $image_end_time - $image_start_time;
                        $image_times[] = $image_duration;
                        ...
                        foreach ($imageUrls as $index => $url) {
                            if (isset($imageInfo[$index])) {
                                $info = $imageInfo[$index];
                                $imageData = downloadImageWithCurl($url, $webp_support);
                                if ($imageData !== false) {
                                    $filePath = $info['path'] . '/' . $info['filename'];
                                    if (file_put_contents($filePath, $imageData)) {
                                        if ($info['type'] === 'main') {
                ...
                if ($is_cron_update) {
                    ...
                    if ($existing_product_id > 0) {
                        ...
                        $batch_products[] = [
                            'data' => array($xurunadi, $xseo, $xurunkodu, $xurunbarkodu, $parabirimi, $xstok, $xfiyat, $xkdv, $xaciklama, $katid, $durum, $tarih, $yildiz, $yeniad, $markaid),
                ...
                // Ürün işleme süresini ölç
                $product_end_time = microtime(true);
                $product_duration = $product_end_time - $product_start_time;
                $product_times[] = $product_duration;
                
                // Her ürün sonrası memory kontrol (WebP dönüştürme için daha sık)
                if ($i % 10 == 0) { // 25'ten 10'a düşürüldü - daha sık kontrol
                    $memory_usage = memory_get_usage(true) / 1024 / 1024; // MB
                    $avg_product_time = array_sum($product_times) / count($product_times);
                    $avg_image_time = !empty($image_times) ? array_sum($image_times) / count($image_times) : 0;
                    
                    file_put_contents('xml_worker_error.txt', "Ürün {$i} tamamlandı - Süre: " . round($product_duration, 3) . "s | Ortalama: " . round($avg_product_time, 3) . "s | Resim Ort: " . round($avg_image_time, 3) . "s | Memory: {$memory_usage}MB\n", FILE_APPEND);
                    
                    if ($memory_usage > 300) { // 350MB'den 300MB'ye düşürüldü - daha agresif temizlik
        ...
        // Final progress raporu - her durumda çalışsın
        ...
        $xml_traffic = 0; // XML trafiği
        $image_traffic = 0; // Resim trafiği
        
        // XML dosyası boyutu
        $xml_size = strlen($xmlData);
        $xml_traffic = $xml_size;
            ...
            foreach ($files as $file) {
                if (is_file($file)) {
                    ...
                    if ($file_time >= $start_time) {
                        ...
                        $image_traffic += $file_size;
                    }
                }
            }
        }
        
        // Toplam trafik = XML + resimler
        $total_traffic = $xml_traffic + $image_traffic;
        ...
        $traffic_report .= "XML Dosya Boyutu: " . formatBytes($xml_traffic) . "\n";
        $traffic_report .= "İndirilen Resim Sayısı: {$successful_concurrent_downloads}\n";
        $traffic_report .= "Toplam Resim Boyutu: " . formatBytes($image_traffic) . "\n";
        ...
        // Trafik verilerini veritabanına kaydet
        $total_traffic_mb = round($total_traffic / (1024 * 1024), 2); // MB cinsinden
        $xml_traffic_mb = round($xml_traffic / (1024 * 1024), 2);
        $image_traffic_mb = round($image_traffic / (1024 * 1024), 2);
        
        $islem_adi = $is_cron_update ? "XML Güncelleme (ID: {$xml_id})" : "XML Yükleme (ID: {$xml_id})";
        $islem_adi .= " - Ürün: {$success_count}, Resim: {$successful_concurrent_downloads}";
        $islem_adi .= " - XML: {$xml_traffic_mb}MB, Resim: {$image_traffic_mb}MB";
        
        saveTrafficData($ozy, $islem_adi, $total_traffic_mb, date('Y-m-d'));
        ...
        $avg_product_time = !empty($product_times) ? array_sum($product_times) / count($product_times) : 0;
        $avg_image_time = !empty($image_times) ? array_sum($image_times) / count($image_times) : 0;
        $max_product_time = !empty($product_times) ? max($product_times) : 0;
        $min_product_time = !empty($product_times) ? min($product_times) : 0;
        
        file_put_contents('xml_progress.txt', "=== DETAYLI PERFORMANS İSTATİSTİKLERİ ===\n", FILE_APPEND);
        file_put_contents('xml_progress.txt', "Ortalama Ürün İşleme Süresi: " . round($avg_product_time, 3) . " saniye\n", FILE_APPEND);
        file_put_contents('xml_progress.txt', "En Hızlı Ürün: " . round($min_product_time, 3) . " saniye\n", FILE_APPEND);
        file_put_contents('xml_progress.txt', "En Yavaş Ürün: " . round($max_product_time, 3) . " saniye\n", FILE_APPEND);
        file_put_contents('xml_progress.txt', "Ortalama Resim İndirme Süresi: " . round($avg_image_time, 3) . " saniye\n", FILE_APPEND);
