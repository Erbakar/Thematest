...
admin_yetki($ozy, $_SESSION['departmanid'], 7); 
if($_GET['duzenle']){
$id = temizle($_GET['duzenle']);
$ekresimid = temizle($_GET['duzenle']);
$sayfam = $ozy->query("select * from kampanyalar where id=$id")->fetch(PDO::FETCH_ASSOC); 
...
if (isset($_POST['guncelle'])) {
  ...
  $yol         = "../resimler/kampanyalar";
   ...
   $id = $_GET['duzenle'];
   $stmt = $ozy->prepare("UPDATE kampanyalar SET adi = ?, aciklama = ?, seo = ?, durum = ?, sira = ?, seodurum = ?, stitle = ?, skey = ?, sdesc = ?, resim = ?, link = ? WHERE id = ?");
   $result2 = $stmt->execute(array($adi, $aciklama, $seo, $durum, $sira, $seodurum, $stitle, $skey, $sdesc, $resimadi, $link, $id));
   
   
   
   if($result2){
	echo '<script type="text/javascript">$(document).ready(function(){toastr["success"]("Başarıyla veriyi güncellediniz.", "Başarılı");});</script>';
	echo '<meta http-equiv="refresh" content="1; url='.$url.'/boss/kampanya/duzenle/'.$id.'">';		
   ...
   $id = $_GET['duzenle'];
   $stmt = $ozy->prepare("UPDATE kampanyalar SET adi = ?, aciklama = ?, seo = ?, durum = ?, sira = ?, seodurum = ?, stitle = ?, skey = ?, sdesc = ?, link = ? WHERE id = ?");
   $result2 = $stmt->execute(array($adi, $aciklama, $seo, $durum, $sira, $seodurum, $stitle, $skey, $sdesc, $link, $id));
...
$q = $ozy->query("SHOW TABLE STATUS LIKE 'kampanyalar'");
...
if (isset($_POST['kaydet'])) {
  ...
  $yol         = "../resimler/kampanyalar";
   ...
   $stmt = $ozy->prepare("INSERT INTO kampanyalar (adi, aciklama, seo, hit, durum, sira, seodurum, stitle, skey, sdesc, tarih, resim, link) 
   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
   $result2 = $stmt->execute(array($adi, $aciklama, $seo, $hit, $durum, $sira, $seodurum, $stitle, $skey, $sdesc, $tarih, $resimadi, $link));
   $id = $ozy->lastInsertId();
   
   
   if($result2){
	echo '<script type="text/javascript">$(document).ready(function(){toastr["success"]("Başarıyla veriyi eklediniz.", "Başarılı");});</script>';
	echo '<meta http-equiv="refresh" content="1; url=tum-kampanyalar">'; 		
...
<div class="wrapper">
        <div class="container-fluid">
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-right">
                            ...
                            <li class="breadcrumb-item active">Kampanya 
							<?php if($_GET['duzenle']){?>
						    Düzenle
						    <?php } else { ?>
                            Ekle
                         ...
                         <!-- Nav tabs -->
                          ...
                          <div class="tab-pane active p-3" id="home-1" role="tabpanel">
                           <div class="form-group row">
                                <label for="example-text-input" class="col-sm-2 col-form-label">Kampanya Linki </br>(Boş bırakırsanız otomatik kampanya sayfasına gider)</label>
                                <div class="col-sm-10">
							  ...
							  <div class="form-group row">
                                <label for="example-text-input" class="col-sm-2 col-form-label">Kampanya Seo Linki </br>(Boş bırakırsanız otomatik seo link oluşacaktır)</label>
                                <div class="col-sm-10">
                            ...
                            <div class="form-group row">
                                <label for="example-text-input" class="col-sm-2 col-form-label">Kampanya Sırası </br>(En başa almak için en yüksek sayıyı veriniz)</label>
                                <div class="col-sm-10">
							...
							<div class="form-group row">
                                <label for="example-text-input" class="col-sm-2 col-form-label">Kampanya Adı</label>
                                <div class="col-sm-10">
                                ...
                                <div class="col-sm-10">
									...
									<?php if($_GET['duzenle']){?>
									<img src="../resimler/kampanyalar/<?php echo $sayfam['resim']; ?>" style="width: 200px; height: 200px;" alt="" />
                                    <?php } else { ?>
			...
			<div class="form-group row">
            <iframe scrolling="no"  src="resimyukle.php?id=<?php echo $ekresimid;?>&alan=kampanyalar" style="width:100%;height:1500px;" frameborder="0"></iframe>
            </div>
