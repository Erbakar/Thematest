...
if($yukledurum=='1'){
echo '<script>window.open("xml_progress_show.php", "_blank", "width=400,height=200");</script>';
file_put_contents('xml_progress.txt', '0|XML dosyası okunuyor...');
...
foreach($obj as $ob) {
   ...
   if($resimtip=='1'){
    ...
    if($i % 50 == 0) {
        file_put_contents('xml_progress.txt', $i.'|'.$i.' ürün işlendi...');
    }
    $i++;
}

file_put_contents('xml_progress.txt', 'done|Tamamlandı! '.$i.' ürün yüklendi.');
$urunadetsayisi = $i-1;
