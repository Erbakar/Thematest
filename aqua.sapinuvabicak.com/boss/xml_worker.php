...
set_time_limit(0);
ini_set('memory_limit', '128M'); // 128MB RAM kullanımı (256MB sunucu için optimize edildi)
...
// Ürün güncelleme fonksiyonu - cron job için (resim indirme olmadan)
function updateExistingProduct($ozy, $product_data, $product_id) {
    try {
        // Önce mevcut verileri al
        $stmt = $ozy->prepare("SELECT adi, seo, urunkodu, urunbarkodu, stok, fiyat, kdv, aciklama, kategori, marka FROM urunler WHERE id = ?");
        ...
        $changes = [];
        $change_log = "=== ÜRÜN GÜNCELLEME DETAYI ===\n";
        $change_log .= "Ürün ID: {$product_id}\n";
        $change_log .= "Tarih: " . date('Y-m-d H:i:s') . "\n";
        $change_log .= "Ürün Kodu: {$product_data['urunkodu']}\n";
        $change_log .= "Ürün Barkodu: {$product_data['urunbarkodu']}\n";
        $change_log .= "----------------------------------------\n";
        ...
        foreach ($fields_to_check as $field => $field_name) {
            $old_value = $old_data[$field] ?? '';
            $new_value = $product_data[$field] ?? '';
            
            if ($old_value != $new_value) {
                $changes[] = $field;
                $change_log .= "{$field_name}: '{$old_value}' → '{$new_value}'\n";
            }
        }
        
        // Eğer değişiklik varsa güncelle ve log'a yaz
        if (!empty($changes)) {
            $stmt = $ozy->prepare("UPDATE urunler SET adi=?, seo=?, urunkodu=?, urunbarkodu=?, stok=?, fiyat=?, kdv=?, aciklama=?, kategori=?, marka=?, idurum=? WHERE id=?");
            $result = $stmt->execute([
                $product_data['adi'],
                $product_data['seo'], 
            ...
            if ($result) {
                $change_log .= "Değişen Alan Sayısı: " . count($changes) . "\n";
                $change_log .= "Değişen Alanlar: " . implode(', ', $changes) . "\n";
                $change_log .= "========================================\n\n";
                file_put_contents('xml_guncelleme_detay.txt', $change_log, FILE_APPEND);
            }
            
            return $result;
        } else {
            // Değişiklik yok, log yazma
            return true; // Başarılı sayılır ama güncelleme yapılmadı
        }
    ...
    try {
        ...
        foreach ($xml->children() as $urun) {
            try {
                ...
                // Güncelleme modunda resim işleme yapma
                $skip_image_processing = $is_cron_update;
                
                if (!$skip_image_processing && $resimtip == '0') { // Tüm ürünler için resim indir
                ...
                if (!$skip_image_processing && $resimtip == '1') { // Tüm ürünler için ek resim işle
                ...
                } else if (!$skip_image_processing) { // Tüm ürünler için normal field modunda resim işle
                ...
                // Eş zamanlı resim indirme işlemi (güncelleme modunda atla)
                if (!$skip_image_processing && !empty($imageUrls)) {
                    try {
                ...
                if ($is_cron_update) {
                    ...
                    if ($existing_product_id > 0) {
                        // Ürün mevcut - sadece verileri güncelle (resim indirme yok)
                        $product_data = [
                            'adi' => $xurunadi,
                            'seo' => $xseo,
                            'urunkodu' => $xurunkodu,
                        ...
                        $update_result = updateExistingProduct($ozy, $product_data, $existing_product_id);
                        if ($update_result) {
                            $update_count++;
                            file_put_contents('xml_progress.txt', "Ürün güncellendi (ID: {$existing_product_id}) - Ad: {$xurunadi} - Fiyat: {$xfiyat} - Stok: {$xstok}\n", FILE_APPEND);
                        }
                    } else {
                        // Ürün mevcut değil - yeni ekle (resim indirme ile)
                        $batch_products[] = [
                            'data' => array(
                                $xurunadi,      // adi
                                $xaciklama,     // aciklama
        ...
        if ($is_cron_update && !empty($xml_product_codes)) {
            ...
            // Final progress raporu - güncelleme modu için
            ...
            file_put_contents('xml_progress.txt', "=== XML GÜNCELLEME TAMAMLANDI ===\n", FILE_APPEND);
            ...
            file_put_contents('xml_progress.txt', "Detaylı Güncelleme Logu: xml_guncelleme_detay.txt\n", FILE_APPEND);
            file_put_contents('xml_progress.txt', "============================\n", FILE_APPEND);
            ...
            // Silme işlemi sonrası final raporu
            ...
            $traffic_data = [
                'islem' => "XML Güncelleme (ID: {$xml_id})",
                'kullanim' => round(($xml_size / 1024 / 1024), 2), // MB cinsinden (sadece XML boyutu)
                'tarih' => date('Y-m-d H:i:s') // Tarih + saat (TR saati)
            ];
