...
class XMLNotificationChecker {
    ...
    checkStatus() {
        ...
        // Path'i dinamik olarak belirle (boss klasörü içinde olup olmadığımızı kontrol et)
        let basePath;
        if (window.location.pathname.includes('/boss/')) {
            // Boss klasörü içindeyse direkt boss/pages yolunu kullan
            basePath = '/boss/pages/xml_process_backend.php';
        } else {
            basePath = 'pages/xml_process_backend.php';
        }
        const fullUrl = basePath.startsWith('http') ? basePath : window.location.origin + basePath.replace(/^\//, '/');
        ...
        .catch(error => {
            // Sessizce hata yakala - sadece geliştirme modunda göster
            if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
                console.warn('XML status kontrolü hatası (geliştirme modu):', error.message);
            }
            // Hata durumunda notification'ı gizle
            this.hideNotification();
        })
        .finally(() => {
            this.isChecking = false;
