<?php
paket_kontrol(["plus", "extreme", "enterprise"]); 
admin_yetki($ozy, $_SESSION['departmanid'], 6);
if (isset($_GET['temizle'])) {

	$id = temizle($_GET['temizle']);
	$sayfasil = $ozy->prepare("delete from tumyorumlar where id='$id' and konu='urunler'");
...
if (isset($_POST['guncelle'])) {
	...
	if ($_POST['durum'] == '1') {
		...
		$urunyorumsayisi = $ozy->prepare("SELECT COUNT(*) FROM tumyorumlar where sayfaid='$sayfaid' and konu='urunler'");
		$urunyorumsayisi->execute();
		$urunyorumsayimis = $urunyorumsayisi->fetchColumn();
		$stmt2 = $ozy->prepare("UPDATE urunler SET yorum = ? WHERE id = ?");
		$result22 = $stmt2->execute(array($urunyorumsayimis, $sayfaid));
	}


	if ($result2) {
		echo '<script type="text/javascript">$(document).ready(function(){toastr["success"]("Başarıyla veriyi güncellediniz.", "Başarılı");});</script>';
		echo '<meta http-equiv="refresh" content="1; url=urun-yorumlari">';
	...
	if ($system['uyeurunonay'] == '1') {
		...
		$mail->Subject = "Ürüne yaptığınız yorum yanıtlandı."; // Konu basligi
		$mail->Body = '
							...
							<table style="font-weight: normal;border-collapse: collapse;border: 0;margin: 0;padding: 0;font-family: Arial, sans-serif;">
								...
								<tr>
									...
									<img class="top-image" src="' . $url . '/mail/webinar-no-text.png" width="560"/><br><br>
										...
										Sitemizdeki <a href="' . $url . '/urun/' . $blogbak['seo'] . '" target="_blank"> ' . $blogbak['adi'] . ' </a> isimli ürüne yaptığınız yoruma cevap geldi.
									
									
			                           </br></br>
			                             <a style="color:#ffffff; background-color: #ff8300;  border: 10px solid #ff8300; border-radius: 3px; text-decoration:none;" target="_blank" href="' . $url . '/urun/' . $blogbak['seo'] . '">Cevabı Görüntüle</a>
									</div>
									</td>
								</tr>
						...
						<td valign="top" align="center" style="border-collapse: collapse;border: 0;margin: 0;padding: 0;-webkit-text-size-adjust: none;color: #555559;font-family: Arial, sans-serif;font-size: 16px;line-height: 26px;">
							<table style="font-weight: normal;border-collapse: collapse;border: 0;margin: 0;padding: 0;font-family: Arial, sans-serif;">
								<tr>
									...
									<td style="border-collapse: collapse;border: 0;margin: 0;padding: 5px;-webkit-text-size-adjust: none;color: #555559;font-family: Arial, sans-serif;font-size: 16px;line-height: 26px;"><a target="_blank" href="' . $ayar['twitter'] . '"><img src="' . $url . '/mail/x-teal.png"></a></td>
	...
	<div class="container-fluid">
		...
		<div class="page-title-box">
			<div class="row align-items-center">
				<div class="col-sm-6">
					<h4 class="page-title">Tüm Ürün Yorumları</h4>
				</div>
				<div class="col-sm-6">
					...
					<div class="card-body">
						...
						<table id="datatable" class="table table-bordered dt-responsive nowrap"
							...
							<tbody>
								...
								<?php $pr = $ozy->query("select * from tumyorumlar where konu='urunler' ORDER BY CASE WHEN cevap = 'cevaplanmadı' THEN 0 ELSE 1 END ASC, id DESC")->fetchAll(PDO::FETCH_ASSOC);
								foreach ($pr as $tr) { ?>
									...
									<tr>
										...
										<td><?php echo $tr['yildiz'] ?></td>
										<td><?php echo strip_tags(mb_substr($tr['yorum'], 0, 15)); ?>...</td>
										<td>
											<?php
											...
											<?php } else { ?>
												<span style="font-size: 13px;"
													class="badge badge-pill badge-danger">Cevaplanmadı</span>
											<?php } ?>
										</td>

										<?php
										$sayfamid = $tr['sayfaid'];
										$ppr = $ozy->query("select * from urunler where id='$sayfamid' ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
										foreach ($ppr as $ptr) { ?>
											<td><a target="_blank" href="../urun/<?php echo $ptr['seo'] ?>">Yoruma Git</a></td>
										<?php } ?>
										<td>
											<?php if ($tr['durum'] == '1') { ?>
										...
										<td>
											...
											<a href="urun-yorumlari?temizle=<?php echo $tr['id']; ?>"
												onclick="return confirm('Silmek istediğinize emin misiniz ?')"
												class="btn btn-sm btn-danger" data-toggle="tooltip"
												data-original-title="Sil"><i class="ti-trash" aria-hidden="true"></i></a>
											<a data-toggle="modal" data-target=".bs-example-modal-lg-<?php echo $tr['id'] ?>"
												class="btn btn-sm btn-success" data-toggle="tooltip"
												data-original-title="Düzenle"><i class="fa fa-edit"
													style="color: white !important;" aria-hidden="true"></i></a>
						...
						<?php $prr = $ozy->query("select * from tumyorumlar where konu='urunler' ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
						foreach ($prr as $trr) { ?>
							...
							<form class="form-horizontal" action="" method="POST" enctype="multipart/form-data">
								...
								<div class="modal fade bs-example-modal-lg-<?php echo $trr['id'] ?>" tabindex="-1"
									role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
									<div class="modal-dialog modal-lg-<?php echo $trr['id'] ?>">
										<div class="modal-content">
											<div class="modal-header">
												<h5 class="modal-title mt-0" id="myLargeModalLabel"><?php echo $trr['adi'] ?>
												</h5>
											...
											<div class="modal-body">
												...
												<div class="form-group row">
													<label for="example-text-input"
														class="col-sm-2 col-form-label">Durumu</label>
													<div class="col-sm-10">
														<input type="checkbox" <?php if ($trr['durum'] == '1') { ?> checked=""
												...
												<div class="form-group row">
													<label for="example-text-input"
														class="col-sm-2 col-form-label">Cevabınız</label>
													<div class="col-sm-10">
														<textarea id="summernote" class="form-control" rows="8"
															name="cevap"><?php echo $trr['cevap']; ?></textarea>
													</div>
