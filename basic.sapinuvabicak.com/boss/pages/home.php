...
<div class="wrapper">
    ...
    <div class="container-fluid">
        ...
        $siparisler = $ozy->query("SELECT * FROM siparis WHERE DATE(tarihson) = CURDATE()")->fetchAll(PDO::FETCH_ASSOC);

        foreach ($siparisler as $siparis) {

            $bst += $siparis['toplamtutar'];
        ...
        $siparislerAy = $ozy->query("SELECT * FROM siparis WHERE MONTH(tarihson) = MONTH(CURDATE()) AND YEAR(tarihson) = YEAR(CURDATE())")->fetchAll(PDO::FETCH_ASSOC);

        foreach ($siparislerAy as $siparis) {

            $ast += $siparis['toplamtutar'];
        ...
        $siparislerYil = $ozy->query("SELECT * FROM siparis WHERE YEAR(tarihson) = YEAR(CURDATE())")->fetchAll(PDO::FETCH_ASSOC);

        foreach ($siparislerYil as $siparis) {

            $yst += $siparis['toplamtutar'];
        ...
        $siparisler = $ozy->query("SELECT * FROM siparis")->fetchAll(PDO::FETCH_ASSOC);

        foreach ($siparisler as $siparis) {

            $tst += $siparis['toplamtutar'];
                                        ...
                                        <table class="table table-bordered dt-responsive nowrap"
                                            ...
                                            <tbody>
                                                ...
                                                function getEmailAccountCount() {
                                                    ...
                                                    $apiUrl = "https://$url:2083/execute/Email/list_pops?api.version=1";
                                                    ...
                                                    $mailList = json_decode($response, true);
                                                ...
                                                function getMarkaCount() {
                                                    global $ozy;
                                                    $markalar = $ozy->query("select * from markalar")->fetchAll(PDO::FETCH_ASSOC);
                                                    return count($markalar);
                                                }

                                                function getPercentageColor($percentage) {
                                                    if ($percentage >= 0 && $percentage < 50) {
                                                        return 'color: #28a745;'; // Yeşil
                                                    } elseif ($percentage >= 50 && $percentage < 80) {
                                                        return 'color: #ffc107;'; // Sarı
                                                    } else {
                                                        return 'color: #dc3545;'; // Kırmızı
                                                    }
                                                }

                                                $urunler = $ozy->query("select * from urunler")->fetchAll(PDO::FETCH_ASSOC);
                                                $urunsayisi = count($urunler);
                                                $markasayisi = getMarkaCount();
                                                $currentEmailCount = getEmailAccountCount();
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php
                                                        ...
                                                        if ($Paket_adi['urunyukleme'] == "0")
                                                            ...
                                                            $percentage = ($urunsayisi / $Paket_adi['urunyukleme']) * 100;
                                                            $color = getPercentageColor($percentage);
                                                            echo $urunsayisi . " / " . number_format($Paket_adi['urunyukleme'], 0, ",", ".") . " Adet (<span style='" . $color . "'>" . round($percentage, 1) . "%</span>)";
                                                        }
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php
                                                        ...
                                                        if ($Paket_adi['markalimit'] == "0")
                                                            ...
                                                            echo $markasayisi . " / Sınırsız";

                                                        else {
                                                            $percentage = ($markasayisi / $Paket_adi['markalimit']) * 100;
                                                            $color = getPercentageColor($percentage);
                                                            echo $markasayisi . " / " . $Paket_adi['markalimit'] . " Adet (<span style='" . $color . "'>" . round($percentage, 1) . "%</span>)";
                                                        }
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php
                                                        ...
                                                        if ($Paket_adi['mailhesabi'] == "0")
                                                            ...
                                                            echo $currentEmailCount . " / Sınırsız";

                                                        else {
                                                            $percentage = ($currentEmailCount / $Paket_adi['mailhesabi']) * 100;
                                                            $color = getPercentageColor($percentage);
                                                            echo $currentEmailCount . " / " . $Paket_adi['mailhesabi'] . " Adet (<span style='" . $color . "'>" . round($percentage, 1) . "%</span>)";
                                                        }
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php
                                                        ...
                                                        if ($Paket_adi['sitedepolama'] == "0")
                                                            ...
                                                            $currentStorageMB = HamSitedepolama2(); // MB cinsinden sayısal değer
                                                            $currentStorageFormatted = sitedepolama(); // Formatlanmış string
                                                            $limitGB = $Paket_adi['sitedepolama']; // GB cinsinden
                                                            $limitMB = $limitGB * 1024; // GB'yi MB'ye çevir
                                                            $percentage = ($currentStorageMB / $limitMB) * 100;
                                                            $color = getPercentageColor($percentage);
                                                            echo $currentStorageFormatted . " / " . number_format($limitGB, 0, ",", ".") . " GB (<span style='" . $color . "'>" . round($percentage, 1) . "%</span>)";
                                                        }
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php
                                                        ...
                                                        if ($Paket_adi['sitetrafik'] == "0")
                                                            ...
                                                            $currentTraffic = Trafik();
                                                            $percentage = ($currentTraffic / $Paket_adi['sitetrafik']) * 100;
                                                            $color = getPercentageColor($percentage);
                                                            echo $currentTraffic . " / " . number_format($Paket_adi['sitetrafik'], 0, ",", ".") . " GB (<span style='" . $color . "'>" . round($percentage, 1) . "%</span>)";
                                                        }
