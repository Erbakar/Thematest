...
set_time_limit(0);
ini_set('memory_limit', '512M'); // 512MB RAM kullanımı (sunucu limiti)
...
function convertToWebP($imageData, $quality = 85) {
    ...
    switch ($mimeType) {
        ...
        case 'image/gif':
            ...
            break;
        case 'image/webp':
            // Zaten WebP formatında, dönüştürmeye gerek yok
            return $imageData;
    }
    ...
    try {
        ...
        foreach ($xml->children() as $urun) {
            try {
                ...
                if (!empty($imageUrls)) {
                    try {
                        $downloadedImages = downloadImagesConcurrently($imageUrls, 25, $webp_support); // WebP desteği ile eş zamanlı indirme
