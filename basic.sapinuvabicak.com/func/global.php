...
// Bot ve gereksiz trafiği engellemek için kontroller
$user_agent = $_SERVER['HTTP_USER_AGENT'] ?? '';
$referer = $_SERVER['HTTP_REFERER'] ?? '';

// Bot user agent'larını kontrol et
...
// Referer kontrolü - sadece kendi sitemizden gelen istekleri kabul et
$is_valid_referer = false;
if (!empty($referer)) {
    $parsed_referer = parse_url($referer);
    $referer_host = $parsed_referer['host'] ?? '';
    if (strpos($referer_host, 'sapinuvabicak.com') !== false || strpos($referer_host, 'localhost') !== false) {
        $is_valid_referer = true;
    }
}

// Bot ise veya geçersiz referer ise işlemi durdur
if ($is_bot || !$is_valid_referer) {
    // Sessizce yönlendir, hata mesajı gösterme
    header("Location: " . $url . "/anasayfa");
    exit;
}

// Aynı IP'den son 1 dakikada kaç karsilastir işlemi yapılmış kontrol et
$recent_count = $ozy->prepare("SELECT COUNT(*) as count FROM karsilastir WHERE kim = ? AND tarih > DATE_SUB(NOW(), INTERVAL 1 MINUTE)");
$recent_count->execute(array($ip));
$recent_result = $recent_count->fetch(PDO::FETCH_ASSOC);

// 1 dakikada 5'ten fazla işlem yapılmışsa engelle
if ($recent_result['count'] > 5) {
    header("Location: " . $url . "/anasayfa");
    exit;
}
